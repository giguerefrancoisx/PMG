...
table = tb.get('SLED')
table_y7 = table.query('DUMMY==\'' + dummy + '\'').filter(items=['SE','MODEL','INSTALL','SLED']+channels)
    ...
    for m in models:
        for t in types:
            for i,ch in enumerate(channels):
                x = {}
                for s in sleds:
                    se = table_y7.query('MODEL==\''+m+'\' and SLED==\''+s+'\' and INSTALL==\'' + t + '\'').index
                    if len(se)==0:
                        x[s] = [np.nan]
                    else:
...
for i, ch in enumerate(channels):
    for t in types:
        ...
        if writefiles:
            mp[ch].to_csv(writename + 'delta_meanprops_' + ch + '_' + t + '.csv')
#        display(ch + ' ' + t + ':')
